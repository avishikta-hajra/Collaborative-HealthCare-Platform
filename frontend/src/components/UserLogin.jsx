...
export default function UserLogin() {
    ...
    return (
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .input-field:focus { border-color:#00C9A7; background:rgba(0,201,167,0.04); box-shadow:0 0 0 4px rgba(0,201,167,0.08); }
        .btn-main { width:100%; background:linear-gradient(135deg,#00C9A7,#0099FF); border:none; color:#fff; font-family:inherit; font-weight:700; font-size:16px; padding:15px; border-radius:12px; cursor:pointer; transition:all 0.25s; letter-spacing:0.01em; }
        .btn-main:hover { transform:translateY(-2px); box-shadow:0 10px 35px rgba(0,201,167,0.3); }
      ...
      `}</style>
            ...
            <div style={{ flex: "0 0 45%", background: "linear-gradient(160deg, #071628 0%, #0A2440 50%, #071628 100%)", padding: "60px 48px", display: "flex", flexDirection: "column", justifyContent: "space-between", position: "relative", overflow: "hidden" }}>
                {/* BG decoration */}
                <div style={{ position: "absolute", top: -100, right: -100, width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(0,201,167,0.08), transparent 70%)", pointerEvents: "none" }} />
                ...
                <div>
                    <a href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none", marginBottom: 64 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#00C9A7,#0099FF)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        <span style={{ color: "#E8F0FE", fontWeight: 700, fontSize: 18 }}>MediLink</span>
                    </a>

                    <div style={{ marginBottom: 20 }}>
                        <span style={{ display: "inline-block", background: "rgba(0,201,167,0.12)", color: "#00C9A7", fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", padding: "4px 12px", borderRadius: 100, border: "1px solid rgba(0,201,167,0.25)", marginBottom: 20 }}>PATIENT PORTAL</span>
                        <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(28px,3vw,40px)", color: "#E8F0FE", lineHeight: 1.15, marginBottom: 16 }}>Your health,<br /><span style={{ fontStyle: "italic", background: "linear-gradient(135deg,#00C9A7,#0099FF)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>always connected.</span></h2>
                        <p style={{ color: "#6A84A0", fontSize: 15, lineHeight: 1.7, maxWidth: 360 }}>Access world-class healthcare from your phone or browser. Consult doctors, track emergencies, and stay informed.</p>
                    </div>
                </div>

                <div>
                    {[
                        { icon: "📹", title: "Instant Video Consultations", desc: "Connect with verified doctors in minutes, 24/7" },
                        { icon: "📍", title: "Find Care Near You", desc: "Locate hospitals, clinics and track ambulances live" },
                        { icon: "🔔", title: "Health Alerts & Reports", desc: "Receive real-time community health notifications" },
                    ].map((f, i) => (
                        <div key={i} className="feature-item">
                            <div style={{ width: 42, height: 42, borderRadius: 11, background: "rgba(0,201,167,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>{f.icon}</div>
                            <div>
                ...
                <div style={{ marginTop: 40, padding: "20px 24px", background: "rgba(0,201,167,0.06)", borderRadius: 14, border: "1px solid rgba(0,201,167,0.15)" }}>
                    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                        <div style={{ width: 42, height: 42, borderRadius: "50%", background: "linear-gradient(135deg,#00C9A7,#0099FF)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>😊</div>
                        <div>
                            <div style={{ color: "#E8F0FE", fontSize: 14, fontWeight: 600, marginBottom: 2 }}>"MediLink saved my father's life."</div>
                            <div style={{ color: "#6A84A0", fontSize: 12 }}>Priya S., patient since 2024</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px clamp(24px,5vw,72px)", animation: "fadeIn 0.5s ease" }}>
                <div style={{ width: "100%", maxWidth: 440 }}>
                    <div style={{ marginBottom: 36 }}>
                        ...
                        <p style={{ color: "#6A84A0", fontSize: 15 }}>Sign in to your patient account</p>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                        {tab === "register" && (
                            <div>
                                ...
                                <input className="input-field" placeholder="Rahul Sharma" />
                            </div>
                        )}
                        <div>
                        ...
                        <div>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                                ...
                                {tab === "login" && <a href="#" style={{ color: "#00C9A7", fontSize: 13, textDecoration: "none" }}>Forgot password?</a>}
                            </div>
                        ...
                        {tab === "login" && (
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                <input type="checkbox" id="remember" style={{ accentColor: "#00C9A7", width: 16, height: 16 }} checked={form.remember} onChange={e => setForm({ ...form, remember: e.target.checked })} />
                    ...
                    <button className="btn-main" style={{ marginTop: 28 }} onClick={handleSubmit} disabled={loading}>
                        ...
                        ) : tab === "login" ? "Sign In to Patient Portal →" : "Create My Account →"}
                    </button>

                    <p style={{ textAlign: "center", marginTop: 24, color: "#4A6080", fontSize: 13 }}>
                        {tab === "login" ? "Don't have an account? " : "Already registered? "}
                        <span onClick={() => setTab(tab === "login" ? "register" : "login")} style={{ color: "#00C9A7", cursor: "pointer", fontWeight: 600 }}>
                    ...
                    <p style={{ textAlign: "center", marginTop: 28, color: "#2D4060", fontSize: 12 }}>
                        Not a patient? <a href="/doctor-login" style={{ color: "#0099FF", textDecoration: "none" }}>Doctor Login</a> · <a href="/admin-login" style={{ color: "#845EF7", textDecoration: "none" }}>Admin Login</a>
                    </p>
                </div>
            </div>
        </div>
