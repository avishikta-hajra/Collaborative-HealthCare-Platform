...
export default function DriverDashboard() {
    ...
    const handleCompleteTrip = () => {
        ...
        // FIX: Block polling alerts immediately to prevent race conditions
        activeTripRef.current = null;
        ...
        .then(res => {
            if (res.ok) {
                ...
                activeTripRef.current = activeTrip; // Restore polling if failed
            }
        })
        .catch(err => {
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-6">
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-blue-100 mb-6 flex items-center justify-between">
                <div>
            ...
            {isOnline && activeTrip && currentStatus === "AVAILABLE" && (
                <div className="bg-white rounded-3xl overflow-hidden shadow-lg border-2 border-red-200 mb-6 animate-fade-in">
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-start mb-5">
                            <div>
                                ...
                                <div className="font-bold text-blue-950 text-[18px] leading-tight">{activeTrip.pickupAddress}</div>
                            </div>
                        </div>
                        ...
                        <button 
                            ...
                            className="w-full bg-blue-700 hover:bg-blue-800 text-white py-4 rounded-xl text-[16px] font-bold shadow-[0_4px_15px_rgba(29,78,216,0.3)] flex items-center justify-center gap-2 cursor-pointer transition-all"
                        >
            ...
            {isOnline && currentStatus === "ON_TRIP" && activeTrip && (
                <div className="bg-white rounded-3xl overflow-hidden shadow-lg border-2 border-blue-400 mb-6">
                    <div className="w-full h-56 bg-slate-200 relative">
                        ...
                        <div className="absolute top-4 left-4 bg-white/95 px-4 py-2 rounded-xl shadow-md border border-slate-200 flex items-center gap-2">
                            ...
                            <span className="text-[14px] font-bold text-blue-950">Live GPS Active</span>
                        </div>
                    </div>
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-center mb-6 pb-6 border-b border-slate-100">
                            <div>
                                ...
                                <div className="font-bold text-blue-950 text-[18px]">{activeTrip.patientName}</div>
                            </div>
                            <a 
                                href={`tel:${activeTrip.patientPhone}`} 
                                className="w-12 h-12 bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100 rounded-full flex justify-center items-center cursor-pointer transition-colors"
                            >
                        ...
                        <button 
                            ...
                            className="w-full bg-blue-950 hover:bg-blue-900 text-white py-4 rounded-xl text-[16px] font-bold flex items-center justify-center gap-2 cursor-pointer shadow-md transition-all"
                        >
            ...
            {isOnline && !activeTrip && currentStatus === "AVAILABLE" && (
                <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-3xl border border-blue-100 shadow-sm">
                    <div className="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center border-2 border-blue-200 mb-6">
                        <Clock className="w-10 h-10 text-blue-600 animate-pulse"/>
                    </div>
                    <h3 className="text-[20px] font-bold text-blue-950 mb-2">Waiting for Dispatches</h3>
