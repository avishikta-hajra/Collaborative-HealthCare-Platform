...
export default function DriverHistory() {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      <h2 className="text-2xl font-bold text-blue-950 mb-6 pl-2">Past Services</h2>
      
      {pastTrips.length === 0 ? (
          <div className="text-center p-10 bg-white rounded-3xl border border-slate-100 text-slate-500 font-medium">
              No completed trips found yet.
          </div>
      ) : (
        <div className="space-y-5">
          {pastTrips.map(trip => (
            <div key={trip.id} className="bg-white rounded-3xl p-6 shadow-sm border border-blue-100 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="text-[13px] font-bold text-slate-400 mb-1">{formatDate(trip.date)}</div>
                  <div className="font-extrabold text-[16px] text-blue-950">{trip.id}</div>
                </div>
              ...
              <div className="space-y-3 mt-4 pt-4 border-t border-slate-100 text-[14px]">
                <div className="flex items-center gap-3 text-slate-700 font-semibold">
                  <div className="w-2.5 h-2.5 rounded-full bg-blue-500 ml-1"></div> {trip.from}
                </div>
                <div className="flex items-center gap-3 text-slate-700 font-semibold">
                  <MapPin className="w-4.5 h-4.5 text-blue-500" /> {trip.to}
                </div>
              </div>
            </div>
          ))}
